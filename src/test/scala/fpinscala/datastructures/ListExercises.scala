...
class ListExercises extends FlatSpec with Matchers {
  ...
  "Exercise 3.16" should "adds 1" in {
    def add1(list: List[Int]): List[Int] = list match {
      case Nil => Nil
      case Construct(head, tail) => Construct(head + 1, add1(tail))
    }
  ...
  "Exercise 3.17" should "map to string" in {
    def mapToString(list: List[Int]): List[String] = list match {
      case Nil => Nil
      case Construct(head, tail) => Construct(head.toString, mapToString(tail))
    }
  ...
  "Exercise 3.18" should "map" in {
    def map[A, B](list: List[A])(f: A => B): List[B] = list match {
      case Nil => Nil
      case Construct(head, tail) => Construct(f(head), map(tail)(f))
    }
